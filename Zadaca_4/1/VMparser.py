class Parser:
    ...
    def parseFile(self, filename):
        ...
        # Citamo oznake iz VM datoteke.
        if not self._readLabels():
            return
    ...
    def _readLabels(self):
        for (line, n) in self._lines:
            ...
            if len(l) == 2 and l[0].strip() == "label":
                if l[1].strip() in self._labelsList:
                    Parser._error("Label", n, "Duplicate label '" + line.strip() + "'")
                    return ""
                else:
                    self._labelsList.append(l[1].strip())
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            try:
                ...
                if len(l) != 3 or value < 0:
                    self._flag = False
                    Parser._error("Parser", n, "Undefined command '" + line.strip() + "'");
                    return ""
            ...
            if l[0] == "label" and len(l) == 2:
                ...
                if l[1].strip() not in self._labelsList:
                    Parser._error("Label", n, "Unknown label '" + l[1].strip() + "'")
                    return ""
                return "//" + " ".join(l) + "\n" + self._goto(l[1], n)
